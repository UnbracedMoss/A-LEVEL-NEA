...
def login_verify():
    ...
    password1 = str(password1) # Assigning the variable "password1" with a string value of password1
    username1 = str(username1) # Assigning the variable "username1" with a string value of username1
    ...
    user_dir = ("C:/Users/achug/Desktop/Files/A Level NEA/tkinter code/Users")
    #Defining the variable "user_dir" with the directory in which all user files will be stored
    os.chdir(user_dir) # Changing the current directory to the user directoy
    list_of_files = os.listdir() # Storing into an array, all the file names of all the files within the user directory
    if username1 in list_of_files: # Initialising a conditional statement based on whether the value of "username1" is in the array of file names
        # If the user has registered, there should be a file name with that username
        # If the user has not registered, there should not be a file name with that username
        file1 = open(username1, "r") # Opening the file with the name of the value of "username1" and assigning its contents to "file1"
        verify = file1.read().splitlines() # Separating the contents and removing the newline comments, then assigning those contents to "verify"
        their_password = verify[1] # Assigning the value of "verify[1]" , which would contain the password to that username, to the variable "their_password"
        boolean_login = (password1 == verify[1]) # Assigning the value of "boolean_logic" of either true or false
        #Depending on whether "password1" and "verify[1]" match (Whether the user entered password and the stored password match)
        if boolean_login == True: # Conditional statement  based on the value of the variable "boolean_logic"
            #If true, it means that the user entered password and the stored password matches
            login_success() # Directing the code to the function login_success()
        else:
            #Condition has not been met (Entered password and stored password does not match)
            password_not_recognised()  # Directing the code to the function password_not_recognised()
    else:
        # Value of "username1" (user entered username) not found in the array containing all the filenames
        user_not_found()
