...
def login_verify():
    ...
    #new code------------------------------------------------------------------------------------------------------------------------
    password1 = str(password1) # Making the variable "password1" a string variable
    username1 = str(username1) # Making the variable "username1" a string variable
    #new code ends------------------------------------------------------------------------------------------------------------------
    ...
    user_dir = ("C:/Users/achug/Desktop/Files/A Level NEA/tkinter code/Users")
    # Defining the variable "user_dir" with the directory in which the user files would be stored
    os.chdir(user_dir) # Changing the directory to the user directory
    list_of_files = os.listdir() # Storing into an array all the file names of the files held within the specific directory
    if username1 in list_of_files: # Intialising a conditional statement, based on whether the value of "username1" is in the array full of filenames
        # If the user has registered, there should be a filename with their username
        # If the user has not registered, there should not be a filename with their username
        file1 = open(username1, "r") # Opening the file with the name of the value of "username1" and assigning its content to "file1"
        verify = file1.read().splitlines() # Separating the contents on the file and removing the newline comments, then assigning those contents to "verify"
        their_password = verify[1] # Assigning the value of "verify[1]", which contains the password to that username and assigning it to the variable "their_password"
    #new code---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        print(password1 == verify[1])
        if (password1 in verify[1]): # Initialising a conditional statement based on whether the value of the user entered password ("password1") is the same as the password stored in the file ("their_password")
            login_success() # Directs the code to the function login_success() if their password matches the password stored in the reccords
        else:
            password_not_recognised() # Directs the code to the function password_not_recognised() if their password does not match the password stored in the records
    else:
        user_not_found()
    #new code ends---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
